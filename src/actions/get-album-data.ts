...
export interface Track {
  ...
  artist: string;
  duration: string;
  url: string; // Renamed from path to url
  totalplays: number;
}
...
export async function getAlbumData(
  ...
  const apiUrl = `https://test.mwonya.com/ios/Requests/endpoints/selectedAlbum.php?page=${page}&albumID=${encodeURIComponent(
    albumId
  )}`;
  ...
  try {
    ...
    const tracksContainer = data.Album.find(
      (item) => item.Tracks && Array.isArray(item.Tracks)
    );
    ...
    const tracks: Track[] = tracksData.map((track) => ({
      ...
      url: track.path || "", // Updated from path to url
      totalplays: track.totalplays || 0,
    }));

    const relatedContainer = data.Album.find(
      (item) => item.ArtistAlbum && Array.isArray(item.ArtistAlbum)
    );
    ...
    console.log(
      `Successfully fetched album: ${albumData.title} by ${albumData.artistName}`
    );
    return albumData;
  } catch (error) {
